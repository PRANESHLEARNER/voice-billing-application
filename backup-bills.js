...
async function backupAllBills() {
  try {
    ...
    // Get all bills for backup
    const bills = await Bill.find({}).sort({ createdAt: -1 });
    console.log(`Found ${bills.length} bills to backup`);
    
    if (bills.length === 0) {
      console.log('No bills to backup.');
      process.exit(0);
    }
    ...
    if (pdfPaths.length === 0) {
      console.log('No PDF files were generated. Aborting backup.');
      process.exit(1);
    }
    ...
    // Step 3: Clean up individual PDF files
    ...
    console.log('All bills remain intact in the database.');
